...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Aurora OS — Spatial operating system for ambient computing" },
      {
        name: "description",
        content:
          "Aurora OS is a gesture-first, multi-window spatial operating system. 60fps everywhere, sub-millisecond input, built for the ambient age.",
      },
      { property: "og:title", content: "Aurora OS — OS for the Ambient Age" },
      {
        property: "og:description",
        content: "Spatial operating system. Gesture-first. Multi-window. 60fps everywhere.",
      },
    ...
    links: [
      ...
      {
        rel: "stylesheet",
        href: "https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800;900&family=JetBrains+Mono:wght@400;500;600&family=Instrument+Serif:ital@0;1&display=swap",
      },
    ],
  }),
  component: Index,
  ...
  return (
    <nav className="sticky top-0 z-50 flex items-center justify-between px-6 md:px-10 py-5 backdrop-blur-md bg-background/40 border-b border-glass-border">
      ...
      <div className="hidden md:flex gap-8 text-sm font-medium text-muted">
        ...
        <a href="#apps" className="hover:text-foreground transition-colors">Apps</a>
        <a href="#input" className="hover:text-foreground transition-colors">Input</a>
        ...
        >
          ...
          >
            ...
            <ul className="py-2 max-h-80 overflow-auto">
              {items.length === 0 && (
                <li className="px-6 py-6 text-sm text-muted">No results.</li>
              )}
              {items.map((i) => (
                <li
                  key={i.label}
    ...
    <section className="max-w-7xl mx-auto px-6 md:px-10 pt-20 md:pt-28 pb-20 relative">
      ...
      >
        ...
        <p className="max-w-xl mx-auto mt-8 text-lg text-muted text-pretty leading-relaxed">
          A spatial operating system that disappears when you don't need it.
          Gesture-first, multi-window, 60fps everywhere — at the speed of thought.
        </p>
...
function LogoMarquee() {
  const logos = [
    "STRIPE", "FIGMA", "LINEAR", "VERCEL", "ARC", "NOTION",
    "RAYCAST", "FRAMER", "PERPLEXITY", "REPLIT", "RESEND", "LOOPS",
  ];
  return (
  ...
  return (
    <section id="architecture" className="max-w-7xl mx-auto px-6 md:px-10 pt-32 md:pt-44">
      ...
      <h2 className="mt-4 text-4xl md:text-6xl font-semibold tracking-tight max-w-2xl">
        An OS that thinks in <em style={{ fontFamily: "Instrument Serif" }} className="text-accent-mint not-italic">three dimensions</em>.
      </h2>
      <motion.div
        initial={{ opacity: 0, y: 24 }}
            ...
            {[0.5, 1, 0.65, 0.35, 0.8, 0.45, 0.9, 0.55, 0.7, 0.4, 0.85].map((h, i) => (
              <div
                ...
                style={{
                  height: `${h * 100}%`,
                  background:
                    i === 4 ? "var(--accent-purple)" : i === 7 ? "var(--accent-mint)" : "rgba(255,255,255,0.12)",
    ...
    },
    {
      ...
      body: (
        <div className="flex items-center justify-center h-full relative">
          ...
          <div className="absolute size-32 rounded-full border border-accent-mint/50" style={{ animationDelay: "-3s" }} />
    ...
    },
    {
      ...
      body: (
        <div className="flex items-end justify-around h-full pb-4">
          {["AL", "JK", "MR", "VP"].map((n, i) => (
            <div key={n} className="flex flex-col items-center gap-2 animate-float" style={{ animationDelay: `${i * -0.5}s` }}>
    ...
    },
    {
      ...
      body: (
        <div className="grid grid-cols-2 gap-3 h-full">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="rounded-xl bg-glass border border-glass-border p-4 flex flex-col justify-between">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="mt-8 flex gap-6 font-mono text-[10px] uppercase tracking-widest text-muted">
              ...
              <span className="text-accent-mint">⌘{tabs.findIndex((t) => t.id === current.id) + 1}</span>
            </div>
          </motion.div>
        </AnimatePresence>
        <motion.div
        ...
        {gestures.map((g, i) => (
          ...
          >
            ...
            <svg viewBox="0 0 120 70" className="w-full h-32 mb-6">
              ...
              <circle
                cx={g.path.match(/L (\d+)/g)?.slice(-1)[0].split(" ")[1] || "110"}
                cy="20"
                r="2.5"
      ...
      <div className="mt-4 grid md:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <p className="text-lg text-muted leading-relaxed max-w-md mb-8">
            Write once in TypeScript. Aurora compiles to spatial windows, AR overlays,
            handheld surfaces, and even legacy 2D. No platform lock-in. No re-writes.
          </p>
...
function CTA() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setStatus("loading");
    try {
      const res = await joinFn({ data: { email: trimmed, source: "landing_cta" } });
      if (!res.ok) {
        setStatus("idle");
        setError(res.error);
      ...
      setStatus("success");
      toast.success(
        res.alreadyJoined ? "You're already on the list." : "You're in. We'll be in touch.",
      );
      setEmail("");
    } catch (err) {
      setStatus("idle");
      ...
      >
        ...
        <div className="relative">
          ...
          <p className="mt-8 md:mt-10 text-lg md:text-xl text-muted max-w-xl mx-auto text-pretty leading-relaxed">
            Aurora is shipping to a small cohort of developers and studios.
            Reserve your seat — we'll reach out personally.
          </p>
          ...
          >
            ...
            >
              ...
              {status === "success" ? "Reserved ✓" : status === "loading" ? "Reserving" : "Request access"}
            </button>
          </form>
